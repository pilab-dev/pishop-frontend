...
export class PiShopClient {
   ...
   */
  async getProducts(pagination?: PaginationInput, filter?: ProductFilterInput): Promise<Product[]> {
    try {
  ...
  async getProductsByIds(ids: string[]): Promise<Product[]> {
    ...
    try {
      ...
      if (result.error) {
        console.error('GraphQL error in getProductsByIds:', result.error)
        return []
      }
   ...
   */
  async updateCartItem(itemId: string, quantity: number): Promise<{ cart: Cart; item: CartItem }> {
    if (!this._cartId) {
    ...
    if (!data?.applyDiscount.success || !data.applyDiscount.cart) {
      ...
      if (error?.code === 'DISCOUNT_APPLY_FAILED') {
        ...
        if (
          ...
          error.message?.includes('no responders available')
        ) {
